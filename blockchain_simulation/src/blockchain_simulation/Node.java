...
public class Node implements Runnable{
	...
	CopyOnWriteArrayList<NodeInterface> neighbors;
	int CACHE_MAX_SIZE = 5000;
	int MINING_DIFFICULTY = 2;
	int TX_PER_BLOCK = 5;
	int BLOCK_CACHE_THRESH = 50;
	...
	int prevTxPoolSize, prevBlockBufferSize;
	float sendSuccessProbability = 0.8f;
	float createTxProbability = 0.5f;
	
	Block curr = null;
	String target = new String(new char[MINING_DIFFICULTY]).replace('\0', '0');
	String prevHash = null; //TODO
	int index = 0; // Index of current existing last block in the block chain.
	
	Random rndm = new Random();
	
	@SuppressWarnings({ "serial", "unchecked" })
	public Node(UUID id) throws Exception{
		cache = new LinkedHashMap<Object, Object>(CACHE_MAX_SIZE, 0.75F, true) {
		...
		prevTxPoolSize = 0; prevBlockBufferSize = 0;
		
		ArrayList<Object> res = Seed.addNeighbors(this);
		neighbors = (CopyOnWriteArrayList<NodeInterface>) res.get(0);
...
//				System.out.println(blockChain);
				...
				System.out.println("ABOOOOOOOOOOOOOOOOOOOOOOORT");
				e.printStackTrace();
			}
			try {
				Thread.sleep(500);
...
//					System.out.println("Duplicate block sent to node: " + this.id.toString());
					...
					if((!blockBuffer.get(i).hash.equals(blockBuffer.get(i).calculateHash()))
							&&(blockBuffer.get(i).prevHash != prevHash || !(complementsBlockBuffer(
									blockBuffer.get(i))))
	...
	private boolean isChainValid() {
		...
		for(int curr = _blockChain.length-1, prev = curr-1; prev >=0; --prev, --curr) {
			...
			if(!_blockChain[curr].equals(_curr.calculateHash())) {
				System.out.println("BLOCK HASHES NOT EQUAAAAAAAAAAAAAAAAAAAAAAAAAL!");
				return false;
			}
			
			if(!_curr.prevHash.equals(_blockChain[prev])) {
				System.out.println("PREVIOUS HASHES NOT EQUAAAAAAAAAAAAAAAAAAAAAAAAAL!");
				return false;
			}
		}
